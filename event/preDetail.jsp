...
<div class="wapper">
    ...
    <div id="fixId" style="position:fixed; background:white; z-index:10; margin-left: 15px;">
         <div class="now-location" style="padding-left: 0px;"><span class="not-im">当前位置：本级事件&nbsp;>&nbsp;待办件&nbsp;>&nbsp;</span><span class="im">事件详情</span></div>
    ...
    <div id="detail">
        <div class="col-md-10 detailLeft">                  
            ...
            <div class="oper-form">
                <%@include file="../common/detail-b.jsp" %>
            </div><!-- oper-form -->
            </div><!-- event-form -->                                                                
...
<script>
var pagePath="event-handle/predoing";
//返回
var closeWin = function(){
    router.setRoute("event/predoing");
}
$(function(){
	//办理操作
	if(localStorage.departmentId == 4){ //指挥中心:交办、批示、直接回复
		$('#assign-radio').parent().css('display','inline');  //交办
		$('#instruct-radio').parent().css('display','inline');  //批示
		$('#reply-radio').parent().css('display','inline');  //直接回复
		$('#isJointDiv').css('display','inline-block');  //只有指挥中心行使“联合执法”功能
		//默认选中交办
	    $('#assign-radio').attr('checked','checked');
	    $('#assign').css('display','block');
	}
	
	if(localStorage.departmentId != 4){ //
		$('#report-radio').parent().css('display','inline');  //上报
		$('#handle-radio').parent().css('display','inline');  //办理
	    $('#endCase-radio').parent().css('display','inline');  //结案申请
		$('#applyDelay-radio').parent().css('display','inline');  //延期申请
		//默认选中办理
        $('#handle-radio').attr('checked','checked');
        $('#handle').css('display','block');
	}
		   
	$.ajax({
		type: 'post',
		url: '../../eventBasicInfo/getAssitStatus',
		data: {
			eventId: id
		},
		dataType: 'json',
		success: function(res){
			//1.先判断是主办部门还是协办部门，协办部门就隐藏“结案申请”、“上报”、“延期申请”按钮
			//2.再判断是否处理，如果未处理，就就隐藏“结案申请”按钮
			if(res.isAssit){
				$('#endCase-radio').parent().css('display','none');
				$('#report-radio').parent().css('display','none');
				$('#applyDelay-radio').parent().css('display','none');
			}else{
	...
	$('#editDetail').click(function(){
		router.setRoute("common/basicInfo?id=" + id + "&backRoute=event/preDetail");
	});
});
</script>
</html>